...
int16  ADC_1_finalArray[ADC_1_NUMBER_OF_CHANNELS];
uint32 ADC_1_initVar = 0u;
static uint8 ADC_1_tempChan;
static uint8 ADC_1_finalChan;
static uint8 ADC_1_tempTD = CY_DMA_INVALID_TD;
static uint8 ADC_1_finalTD = CY_DMA_INVALID_TD;


/****************************************************************************
* Function Name: ADC_1_Disable()
*****************************************************************************
*
* Summary:
...
*
****************************************************************************/
void ADC_1_Disable(void)
{
    ADC_1_CONTROL_REG &= ((uint8)(~ADC_1_BASE_COMPONENT_ENABLE));

    (void) CyDmaChDisable(ADC_1_tempChan);
    CyDmaTdFree(ADC_1_tempTD);
    ADC_1_tempTD = CY_DMA_INVALID_TD;

    (void) CyDmaChDisable(ADC_1_finalChan);
    CyDmaTdFree(ADC_1_finalTD);
    ADC_1_finalTD = CY_DMA_INVALID_TD;
}
...
void ADC_1_Init(void)
{
    ...
    ADC_1_tempChan = ADC_1_TempBuf_DmaInitialize(ADC_1_TEMP_BYTES_PER_BURST,
        ADC_1_REQUEST_PER_BURST, (uint16)(HI16(CYDEV_PERIPH_BASE)), (uint16)(HI16(CYDEV_SRAM_BASE)));

    /* Init DMA, (ADC_1_NUMBER_OF_CHANNELS << 1u) bytes bursts, each burst requires a request */
    ADC_1_finalChan = ADC_1_FinalBuf_DmaInitialize((uint8)ADC_1_FINAL_BYTES_PER_BURST,
        ADC_1_REQUEST_PER_BURST, (uint16)(HI16(CYDEV_SRAM_BASE)), (uint16)(HI16(CYDEV_SRAM_BASE)));

    #if(ADC_1_IRQ_REMOVE == 0u)
        /* Set the ISR to point to the ADC_1_IRQ Interrupt. */
        ADC_1_IRQ_SetVector(&ADC_1_ISR);
        /* Set the priority. */
        ADC_1_IRQ_SetPriority((uint8)ADC_1_INTC_NUMBER);
    #endif   /* End ADC_1_IRQ_REMOVE */
...
void ADC_1_Enable(void)
{
    ...
    static int16 ADC_1_tempArray[ADC_1_NUMBER_OF_CHANNELS];
    
    (void)CyDmaClearPendingDrq(ADC_1_tempChan);
    (void)CyDmaClearPendingDrq(ADC_1_finalChan);
    ...
    */
    ...
    if (ADC_1_tempTD == DMA_INVALID_TD)
    {
        ADC_1_tempTD = CyDmaTdAllocate();
    }

    (void) CyDmaTdSetConfiguration(ADC_1_tempTD, ADC_1_TEMP_TRANSFER_COUNT,
        ADC_1_tempTD, ((uint8)ADC_1_TempBuf__TD_TERMOUT_EN | (uint8)TD_INC_DST_ADR));

    /* From the SAR to the TempArray */
    (void) CyDmaTdSetAddress(ADC_1_tempTD, (uint16)(LO16((uint32)ADC_1_SAR_DATA_ADDR_0)),
        (uint16)(LO16((uint32)ADC_1_tempArray)));

    /* Associate the TD with the channel */
    (void) CyDmaChSetInitialTd(ADC_1_tempChan, ADC_1_tempTD);
    ...
    */
    ...
    if (ADC_1_finalTD == DMA_INVALID_TD)
    {
        ADC_1_finalTD = CyDmaTdAllocate();
    }
    
    (void) CyDmaTdSetConfiguration(ADC_1_finalTD, (ADC_1_FINAL_BYTES_PER_BURST),
        ADC_1_finalTD, ((uint8)(ADC_1_FinalBuf__TD_TERMOUT_EN) | (uint8)TD_INC_SRC_ADR |
            (uint8)TD_INC_DST_ADR));

    /* From the the TempArray to Final Array */
    (void) CyDmaTdSetAddress(ADC_1_finalTD, (uint16)(LO16((uint32)ADC_1_tempArray)),
        (uint16)(LO16((uint32)ADC_1_finalArray)));

    /* Associate the TD with the channel */
    (void) CyDmaChSetInitialTd(ADC_1_finalChan, ADC_1_finalTD);
    
    (void) CyDmaChEnable(ADC_1_tempChan, 1u);
    (void) CyDmaChEnable(ADC_1_finalChan, 1u);

    /* Enable Counter and give Enable pulse to set an address of the last channel */
    enableInterrupts = CyEnterCriticalSection();
    ADC_1_CYCLE_COUNTER_AUX_CONTROL_REG |= ((uint8)(ADC_1_CYCLE_COUNTER_ENABLE));
    CyExitCriticalSection(enableInterrupts);

    /* Enable FSM of the Base Component */
    ADC_1_CONTROL_REG |= ((uint8)(ADC_1_BASE_COMPONENT_ENABLE));
    ADC_1_CONTROL_REG |= ((uint8)(ADC_1_LOAD_COUNTER_PERIOD));
...
void ADC_1_Stop(void)
{
...
#if(ADC_1_SAMPLE_MODE != ADC_1_SAMPLE_MODE_HW_TRIGGERED)
    ...
    *
    * Side Effects:
    *  Calling ADC_1_StartConvert() disables the external SOC pin.
    *
    * Reentrant:
    *  No.
    *
    *******************************************************************************/
    void ADC_1_StartConvert(void)
    {
            ...
            ADC_1_CONTROL_REG |= ((uint8)(ADC_1_SOFTWARE_SOC_PULSE));
        ...
        #endif /*
                 #if(ADC_1_SAMPLE_MODE !=
                 ADC_1_SAMPLE_MODE_FREE_RUNNING)
               */
    }
    ...
    *******************************************************************************/
    ...
    {
        ADC_1_SAR_StopConvert();
    }

#endif /* ADC_1_SAMPLE_MODE != ADC_1_SAMPLE_MODE_HW_TRIGGERED */
...
*   Values:
*         - ADC_1_RETURN_STATUS      - Immediately returns the 
*                                                 status
*         - ADC_1_WAIT_FOR_RESULT    - Does not return a result 
...
*
*******************************************************************************/
uint32 ADC_1_IsEndConversion(uint8 retMode)
{
    uint8 status;

    do
    {
      status = ADC_1_STATUS_REG;
    } while ((status == 0u) && (retMode == ADC_1_WAIT_FOR_RESULT));
...
*
*******************************************************************************/
int16 ADC_1_GetResult16(uint16 chan)
{
    return (ADC_1_finalArray[ADC_1_GET_RESULT_INDEX_OFFSET - chan] - ADC_1_SAR_shift);
}


/*******************************************************************************
* Function Name: ADC_1_GetAdcResult
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
int16 ADC_1_GetAdcResult(void)
{
    return (ADC_1_SAR_GetResult16());
}
...
{
    ADC_1_SAR_SetOffset((int16)offset);
}
...
*  ADC_1_StopConvert(), change the resolution, then restart the
*  conversions with ADC_1_StartConvert().
*  If you decide not to stop conversions before calling this API, you
*  should use ADC_1_IsEndConversion() to wait until conversion is 
...
*  ADC_1_SetResolution() is called before data is valid again.
*  Affects ADC_1_CountsTo_Volts(), ADC_1_CountsTo_mVolts(), 
*  and ADC_1_CountsTo_uVolts() by calculating the correct conversion 
...
{
    ADC_1_SAR_SetResolution(resolution);
}
...
{
    ADC_1_SAR_SetGain((int16)adcGain);
}
...
* Side Effects:
*  Affects ADC_1_CountsTo_Volts(), ADC_1_CountsTo_mVolts(),
*  ADC_1_CountsTo_uVolts() by supplying the correct conversion 
...
{
    ADC_1_SAR_SetScaledGain(adcGain);
}
...
*
*******************************************************************************/
int32 ADC_1_CountsTo_mVolts(int16 adcCounts)
{
    return ((int32) ADC_1_SAR_CountsTo_mVolts(adcCounts));
}
...
{
    return (ADC_1_SAR_CountsTo_uVolts(adcCounts));
}
...
{
    return (ADC_1_SAR_CountsTo_Volts(adcCounts));
}
