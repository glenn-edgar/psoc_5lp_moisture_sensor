...
static uint16 set_moisture_sampling_rate( uint8 *buffer )
{
    ...
    temp32 = temp16;
    write_32_eeprom(  EEPROM_SAMPLING_RATE,(uint8 *) &temp32 );
    return 0;
}
...
static uint16 clear_moisture_log_data( uint8 *buffer )
{
    ...
    temp32 = temp16;
    write_32_eeprom( EEPROM_LOGGING_INDEX ,(uint8 *) &temp32 );
    return 1;
}
...
static uint16 update_moisture_sensor_configuration( uint8 *buffer )
{
    ...
    Em_EEPROM_1_Write(buffer, (uint8 *) moisture_configuration_data, sizeof(update_moisture_sensor_configuration));
...
{
    cf_send_event( CF_MOISTURE_READING, 0 );
    return 1;
}
...
#define TABLE_LENGTH sizeof(type_16_access_functions)/sizeof(type_16_access_functions)
