/*******************************************************************************
* File Name: ad_process_clock_1.c
* Version 2.20
*
*  Description:
...
/*******************************************************************************
* Function Name: ad_process_clock_1_Start
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ad_process_clock_1_Start(void) 
{
    /* Set the bit to enable the clock. */
    ad_process_clock_1_CLKEN |= ad_process_clock_1_CLKEN_MASK;
	ad_process_clock_1_CLKSTBY |= ad_process_clock_1_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: ad_process_clock_1_Stop
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ad_process_clock_1_Stop(void) 
{
    /* Clear the bit to disable the clock. */
    ad_process_clock_1_CLKEN &= (uint8)(~ad_process_clock_1_CLKEN_MASK);
	ad_process_clock_1_CLKSTBY &= (uint8)(~ad_process_clock_1_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: ad_process_clock_1_StopBlock
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ad_process_clock_1_StopBlock(void) 
{
    if ((ad_process_clock_1_CLKEN & ad_process_clock_1_CLKEN_MASK) != 0u)
    {
...
#if defined(ad_process_clock_1__CFG3)
        CLK_DIST_AMASK = ad_process_clock_1_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = ad_process_clock_1_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* ad_process_clock_1__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(ad_process_clock_1_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(ad_process_clock_1_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: ad_process_clock_1_StandbyPower
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ad_process_clock_1_StandbyPower(uint8 state) 
{
    if(state == 0u)
    {
        ad_process_clock_1_CLKSTBY &= (uint8)(~ad_process_clock_1_CLKSTBY_MASK);
...
/*******************************************************************************
* Function Name: ad_process_clock_1_SetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ad_process_clock_1_SetDividerRegister(uint16 clkDivider, uint8 restart)
                                
{
    uint8 enabled;

    uint8 currSrc = ad_process_clock_1_GetSourceRegister();
    uint16 oldDivider = ad_process_clock_1_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = ad_process_clock_1_CLKEN & ad_process_clock_1_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(ad_process_clock_1_DIV_PTR, clkDivider);
                ad_process_clock_1_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                ad_process_clock_1_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(ad_process_clock_1_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((ad_process_clock_1_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(ad_process_clock_1_DIV_PTR, clkDivider);
				ad_process_clock_1_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: ad_process_clock_1_GetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint16 ad_process_clock_1_GetDividerRegister(void) 
{
    return CY_GET_REG16(ad_process_clock_1_DIV_PTR);
}


/*******************************************************************************
* Function Name: ad_process_clock_1_SetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ad_process_clock_1_SetModeRegister(uint8 modeBitMask) 
{
    ad_process_clock_1_MOD_SRC |= modeBitMask & (uint8)ad_process_clock_1_MODE_MASK;
}


/*******************************************************************************
* Function Name: ad_process_clock_1_ClearModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ad_process_clock_1_ClearModeRegister(uint8 modeBitMask) 
{
    ad_process_clock_1_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(ad_process_clock_1_MODE_MASK));
}


/*******************************************************************************
* Function Name: ad_process_clock_1_GetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 ad_process_clock_1_GetModeRegister(void) 
{
    return ad_process_clock_1_MOD_SRC & (uint8)(ad_process_clock_1_MODE_MASK);
}


/*******************************************************************************
* Function Name: ad_process_clock_1_SetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ad_process_clock_1_SetSourceRegister(uint8 clkSource) 
{
    uint16 currDiv = ad_process_clock_1_GetDividerRegister();
    uint8 oldSrc = ad_process_clock_1_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        ad_process_clock_1_MOD_SRC |= CYCLK_SSS;
        ad_process_clock_1_MOD_SRC =
            (ad_process_clock_1_MOD_SRC & (uint8)(~ad_process_clock_1_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: ad_process_clock_1_GetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 ad_process_clock_1_GetSourceRegister(void) 
{
    return ad_process_clock_1_MOD_SRC & ad_process_clock_1_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: ad_process_clock_1_SetPhaseRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ad_process_clock_1_SetPhaseRegister(uint8 clkPhase) 
{
    ad_process_clock_1_PHASE = clkPhase & ad_process_clock_1_PHASE_MASK;
}


/*******************************************************************************
* Function Name: ad_process_clock_1_GetPhase
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 ad_process_clock_1_GetPhaseRegister(void) 
{
    return ad_process_clock_1_PHASE & ad_process_clock_1_PHASE_MASK;
}
