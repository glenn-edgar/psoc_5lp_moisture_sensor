/*******************************************************************************
* File Name: moisture_interval_clock.c
* Version 2.20
*
*  Description:
...
#include "moisture_interval_clock.h"
...
/*******************************************************************************
* Function Name: moisture_interval_clock_Start
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void moisture_interval_clock_Start(void) 
{
    /* Set the bit to enable the clock. */
    moisture_interval_clock_CLKEN |= moisture_interval_clock_CLKEN_MASK;
	moisture_interval_clock_CLKSTBY |= moisture_interval_clock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: moisture_interval_clock_Stop
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void moisture_interval_clock_Stop(void) 
{
    /* Clear the bit to disable the clock. */
    moisture_interval_clock_CLKEN &= (uint8)(~moisture_interval_clock_CLKEN_MASK);
	moisture_interval_clock_CLKSTBY &= (uint8)(~moisture_interval_clock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: moisture_interval_clock_StopBlock
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void moisture_interval_clock_StopBlock(void) 
{
    if ((moisture_interval_clock_CLKEN & moisture_interval_clock_CLKEN_MASK) != 0u)
    {
...
#if defined(moisture_interval_clock__CFG3)
        CLK_DIST_AMASK = moisture_interval_clock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = moisture_interval_clock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* moisture_interval_clock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(moisture_interval_clock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(moisture_interval_clock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: moisture_interval_clock_StandbyPower
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void moisture_interval_clock_StandbyPower(uint8 state) 
{
    if(state == 0u)
    {
        moisture_interval_clock_CLKSTBY &= (uint8)(~moisture_interval_clock_CLKSTBY_MASK);
...
/*******************************************************************************
* Function Name: moisture_interval_clock_SetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void moisture_interval_clock_SetDividerRegister(uint16 clkDivider, uint8 restart)
                                
{
    uint8 enabled;

    uint8 currSrc = moisture_interval_clock_GetSourceRegister();
    uint16 oldDivider = moisture_interval_clock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = moisture_interval_clock_CLKEN & moisture_interval_clock_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(moisture_interval_clock_DIV_PTR, clkDivider);
                moisture_interval_clock_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                moisture_interval_clock_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(moisture_interval_clock_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((moisture_interval_clock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(moisture_interval_clock_DIV_PTR, clkDivider);
				moisture_interval_clock_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: moisture_interval_clock_GetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint16 moisture_interval_clock_GetDividerRegister(void) 
{
    return CY_GET_REG16(moisture_interval_clock_DIV_PTR);
}


/*******************************************************************************
* Function Name: moisture_interval_clock_SetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void moisture_interval_clock_SetModeRegister(uint8 modeBitMask) 
{
    moisture_interval_clock_MOD_SRC |= modeBitMask & (uint8)moisture_interval_clock_MODE_MASK;
}


/*******************************************************************************
* Function Name: moisture_interval_clock_ClearModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void moisture_interval_clock_ClearModeRegister(uint8 modeBitMask) 
{
    moisture_interval_clock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(moisture_interval_clock_MODE_MASK));
}


/*******************************************************************************
* Function Name: moisture_interval_clock_GetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 moisture_interval_clock_GetModeRegister(void) 
{
    return moisture_interval_clock_MOD_SRC & (uint8)(moisture_interval_clock_MODE_MASK);
}


/*******************************************************************************
* Function Name: moisture_interval_clock_SetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void moisture_interval_clock_SetSourceRegister(uint8 clkSource) 
{
    uint16 currDiv = moisture_interval_clock_GetDividerRegister();
    uint8 oldSrc = moisture_interval_clock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        moisture_interval_clock_MOD_SRC |= CYCLK_SSS;
        moisture_interval_clock_MOD_SRC =
            (moisture_interval_clock_MOD_SRC & (uint8)(~moisture_interval_clock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: moisture_interval_clock_GetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 moisture_interval_clock_GetSourceRegister(void) 
{
    return moisture_interval_clock_MOD_SRC & moisture_interval_clock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: moisture_interval_clock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void moisture_interval_clock_SetPhaseRegister(uint8 clkPhase) 
{
    moisture_interval_clock_PHASE = clkPhase & moisture_interval_clock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: moisture_interval_clock_GetPhase
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 moisture_interval_clock_GetPhaseRegister(void) 
{
    return moisture_interval_clock_PHASE & moisture_interval_clock_PHASE_MASK;
}
