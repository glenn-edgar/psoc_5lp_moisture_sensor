/*******************************************************************************
* File Name: modbus_symbol_clock.c
* Version 2.20
*
*  Description:
...
/*******************************************************************************
* Function Name: modbus_symbol_clock_Start
********************************************************************************
*
* Summary:
...
void modbus_symbol_clock_Start(void) 
{
    ...
    modbus_symbol_clock_CLKEN |= modbus_symbol_clock_CLKEN_MASK;
	modbus_symbol_clock_CLKSTBY |= modbus_symbol_clock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: modbus_symbol_clock_Stop
********************************************************************************
*
* Summary:
...
void modbus_symbol_clock_Stop(void) 
{
    ...
    modbus_symbol_clock_CLKEN &= (uint8)(~modbus_symbol_clock_CLKEN_MASK);
	modbus_symbol_clock_CLKSTBY &= (uint8)(~modbus_symbol_clock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: modbus_symbol_clock_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((modbus_symbol_clock_CLKEN & modbus_symbol_clock_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = modbus_symbol_clock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = modbus_symbol_clock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* modbus_symbol_clock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(modbus_symbol_clock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(modbus_symbol_clock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: modbus_symbol_clock_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: modbus_symbol_clock_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = modbus_symbol_clock_GetSourceRegister();
    uint16 oldDivider = modbus_symbol_clock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = modbus_symbol_clock_CLKEN & modbus_symbol_clock_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(modbus_symbol_clock_DIV_PTR, clkDivider);
                modbus_symbol_clock_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                modbus_symbol_clock_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(modbus_symbol_clock_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((modbus_symbol_clock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(modbus_symbol_clock_DIV_PTR, clkDivider);
				modbus_symbol_clock_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: modbus_symbol_clock_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(modbus_symbol_clock_DIV_PTR);
}


/*******************************************************************************
* Function Name: modbus_symbol_clock_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    modbus_symbol_clock_MOD_SRC |= modeBitMask & (uint8)modbus_symbol_clock_MODE_MASK;
}


/*******************************************************************************
* Function Name: modbus_symbol_clock_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    modbus_symbol_clock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(modbus_symbol_clock_MODE_MASK));
}


/*******************************************************************************
* Function Name: modbus_symbol_clock_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return modbus_symbol_clock_MOD_SRC & (uint8)(modbus_symbol_clock_MODE_MASK);
}


/*******************************************************************************
* Function Name: modbus_symbol_clock_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = modbus_symbol_clock_GetDividerRegister();
    uint8 oldSrc = modbus_symbol_clock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        modbus_symbol_clock_MOD_SRC |= CYCLK_SSS;
        modbus_symbol_clock_MOD_SRC =
            (modbus_symbol_clock_MOD_SRC & (uint8)(~modbus_symbol_clock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: modbus_symbol_clock_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return modbus_symbol_clock_MOD_SRC & modbus_symbol_clock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: modbus_symbol_clock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    modbus_symbol_clock_PHASE = clkPhase & modbus_symbol_clock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: modbus_symbol_clock_GetPhase
********************************************************************************
*
* Summary:
...
{
    return modbus_symbol_clock_PHASE & modbus_symbol_clock_PHASE_MASK;
}
