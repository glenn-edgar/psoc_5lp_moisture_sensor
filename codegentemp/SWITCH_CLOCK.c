...
void SWITCH_CLOCK_Start(void) 
{
    ...
    SWITCH_CLOCK_CLKEN |= SWITCH_CLOCK_CLKEN_MASK;
	SWITCH_CLOCK_CLKSTBY |= SWITCH_CLOCK_CLKSTBY_MASK;
}
...
void SWITCH_CLOCK_Stop(void) 
{
    ...
    SWITCH_CLOCK_CLKEN &= (uint8)(~SWITCH_CLOCK_CLKEN_MASK);
	SWITCH_CLOCK_CLKSTBY &= (uint8)(~SWITCH_CLOCK_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: SWITCH_CLOCK_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((SWITCH_CLOCK_CLKEN & SWITCH_CLOCK_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = SWITCH_CLOCK_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = SWITCH_CLOCK_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* SWITCH_CLOCK__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(SWITCH_CLOCK_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(SWITCH_CLOCK_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: SWITCH_CLOCK_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: SWITCH_CLOCK_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = SWITCH_CLOCK_GetSourceRegister();
    uint16 oldDivider = SWITCH_CLOCK_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = SWITCH_CLOCK_CLKEN & SWITCH_CLOCK_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(SWITCH_CLOCK_DIV_PTR, clkDivider);
                SWITCH_CLOCK_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                SWITCH_CLOCK_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(SWITCH_CLOCK_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((SWITCH_CLOCK_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
{
    return CY_GET_REG16(SWITCH_CLOCK_DIV_PTR);
}


/*******************************************************************************
* Function Name: SWITCH_CLOCK_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    SWITCH_CLOCK_MOD_SRC |= modeBitMask & (uint8)SWITCH_CLOCK_MODE_MASK;
}


/*******************************************************************************
* Function Name: SWITCH_CLOCK_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    SWITCH_CLOCK_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(SWITCH_CLOCK_MODE_MASK));
}


/*******************************************************************************
* Function Name: SWITCH_CLOCK_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return SWITCH_CLOCK_MOD_SRC & (uint8)(SWITCH_CLOCK_MODE_MASK);
}


/*******************************************************************************
* Function Name: SWITCH_CLOCK_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = SWITCH_CLOCK_GetDividerRegister();
    uint8 oldSrc = SWITCH_CLOCK_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        SWITCH_CLOCK_MOD_SRC |= CYCLK_SSS;
        SWITCH_CLOCK_MOD_SRC =
            (SWITCH_CLOCK_MOD_SRC & (uint8)(~SWITCH_CLOCK_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: SWITCH_CLOCK_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return SWITCH_CLOCK_MOD_SRC & SWITCH_CLOCK_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: SWITCH_CLOCK_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    SWITCH_CLOCK_PHASE = clkPhase & SWITCH_CLOCK_PHASE_MASK;
}


/*******************************************************************************
* Function Name: SWITCH_CLOCK_GetPhase
********************************************************************************
*
* Summary:
...
{
    return SWITCH_CLOCK_PHASE & SWITCH_CLOCK_PHASE_MASK;
}
